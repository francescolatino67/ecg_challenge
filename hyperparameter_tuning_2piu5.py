...
def train_and_evaluate(signals, tabular_data, output_dir, num_epochs=5, batch_size=32, n_splits=5, threshold=0.5):
    """
    ...
    results_str = f"""
    Accuracy Test:  {accuracy_list_all_folds}
    Accuracy Train: {accuracy_list_all_folds_train}
    F1 Test:        {f1_list_all_folds}
    F1 Train:       {f1_list_all_folds_train}
    Sens Test:      {sensitivity_list_all_folds}
    Sens Train:     {sensitivity_list_all_folds_train}
    Spec Test:      {specificity_list_all_folds}
    Spec Train:     {specificity_list_all_folds_train}
    AUC Test:       {auc_score_list_all_folds}
    AUC Train:      {auc_score_list_all_folds_train}
    Test Loss (epoca max F1):  {test_loss_max}
    Train Loss (epoca max F1): {train_loss_max}
    Epoche selezionate per fold: {epochs_all_fold}
    """
    print(results_str)
...
# Eseguo training + cross-validation (Grid Search)

# Definisci le liste di parametri da testare
EPOCHS_LIST = [30, 50, 100]           # Per vedere convergenza a lungo termine
SPLITS_LIST = [1,2, 5, 10]            # Cross-validation a 5 o 10 fold
THRESHOLDS_LIST = [0.4, 0.5, 0.6] # Soglia di binarizzazione
BATCH_SIZES_LIST = [16, 32, 64]   # Influenza la stabilità del gradiente

# Crea una cartella principale per questo set di esperimenti con timestamp
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
main_experiment_dir = os.path.join(ROOT_PATH, "results_2plus5", f"experiment_{timestamp}")
os.makedirs(main_experiment_dir, exist_ok=True)
print(f"--- Starting Grid Search (2+5) ---")
print(f"Results will be saved to: {main_experiment_dir}")

for epoch, split, thresh, batch in itertools.product(EPOCHS_LIST, SPLITS_LIST, THRESHOLDS_LIST, BATCH_SIZES_LIST):
    run_name = f"epochs{epoch}_splits{split}_thresh{thresh}_batch{batch}"
    print(f"\n>>> Running configuration: {run_name}")
    
    # Crea sottocartella per questa specifica configurazione
    current_output_dir = os.path.join(main_experiment_dir, run_name)
    
    try:
        train_and_evaluate(
            signals=signals,
            tabular_data=tabular_data_balanced,
            output_dir=current_output_dir, # Passa la directory di output
            num_epochs=epoch,
            batch_size=batch,
            n_splits=split,
            threshold=thresh
...
print(f"All results saved in: {main_experiment_dir}")