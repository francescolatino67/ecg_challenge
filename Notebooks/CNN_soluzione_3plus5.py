...
# Se usi Colab con Drive, sostituisci questi path con i tuoi (es. "/content/drive/MyDrive/...").
...
###############################################
# DATASET PYTORCH (ECG 2D + TABELLARE)
###############################################

class ECG2DDataset(Dataset):
    """
    Dataset PyTorch per il modello 3+5:
...
class TabularBranch(nn.Module):
    """
    Ramo MLP per feature tabellari (opzione 5).
    """
...
class ECG2DModel3Plus5(nn.Module):
    """
    Opzione 3+5: ECG 2D + ramo tabellare.
    ...
    def __init__(self, tab_in_features,
                 ecg_out_dim=256, tab_hidden=32, dropout=0.3):
        super().__init__()
...
###############################################
# TRAINING + CROSS-VALIDATION (3+5)
###############################################

def train_and_evaluate_3plus5(signals, tabular_data,
                              num_epochs=5, batch_size=32,
                              n_splits=10, threshold=0.5):
    """
    Cross-validation stratificata per l'opzione 3+5:
      - ECG come immagine 2D
      - tabellare con MLP
    """
    ...
    for fold_idx, (train_index, test_index) in enumerate(
        ...
        for epoch in tqdm(range(num_epochs), desc=f"Fold {fold_idx+1}/{n_splits}"):
            ...
            for tab_batch, ecg_img_batch, lab_batch in train_loader:
                tab_batch = tab_batch.to(device)
                ecg_img_batch = ecg_img_batch.to(device)
                lab_batch = lab_batch.to(device)

                optimizer.zero_grad()
                outputs = model(tab_batch, ecg_img_batch)  # (B,1)
            ...
            with torch.no_grad():
                for tab_batch, ecg_img_batch, lab_batch in test_loader:
                    tab_batch = tab_batch.to(device)
                    ecg_img_batch = ecg_img_batch.to(device)
                    lab_batch = lab_batch.to(device)

                    outputs = model(tab_batch, ecg_img_batch)
                    loss = criterion(outputs, lab_batch)
                    test_loss += loss.item()
        ...
        # Epoca di F1 max per questo fold
    ...
    plt.title("ROC Curve per tutti i fold (3+5)")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Learning Curves (Train/Test Loss per Fold) - 3+5")
...
if __name__ == "__main__":
    ...
    # Training + CV per il modello 3+5
    train_and_evaluate_3plus5(
        signals=signals,
