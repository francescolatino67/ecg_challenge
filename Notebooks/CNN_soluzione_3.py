...
# Se usi Colab con Drive, sostituisci questi path con quelli del tuo Drive.
...
###############################################
# SEGMENTAZIONE DEL SEGNALE (1D -> finestra)
###############################################
...
###############################################
# DATASET PYTORCH (2D INPUT, SOLO ECG)
###############################################
...
###############################################
# TRAINING + CROSS-VALIDATION (SOLO 3)
###############################################

def train_and_evaluate_3_only(signals, tabular_data,
                              num_epochs=5, batch_size=32,
                              n_splits=10, threshold=0.5):
    """
    Cross-validation stratificata per l'opzione 3 (solo ECG 2D).
    tabular_data serve solo per ricavare le etichette sport_ability.
    """
    ...
    for fold_idx, (train_index, test_index) in enumerate(
        ...
        # Split tabellare (solo per le etichette)
        ...
        # Dataset & DataLoader 2D (senza tabellare)
        train_dataset = ECG2DDataset(ecg_train_segments, Y_train)
        test_dataset  = ECG2DDataset(ecg_test_segments,  Y_test)
        ...
        for epoch in tqdm(range(num_epochs), desc=f"Fold {fold_idx+1}/{n_splits}"):
            ...
            for ecg_img_batch, lab_batch in train_loader:
                ecg_img_batch = ecg_img_batch.to(device)
                lab_batch = lab_batch.to(device)

                optimizer.zero_grad()
                outputs = model(ecg_img_batch)  # (B,1)
            ...
            with torch.no_grad():
                for ecg_img_batch, lab_batch in test_loader:
                    ecg_img_batch = ecg_img_batch.to(device)
                    lab_batch = lab_batch.to(device)

                    outputs = model(ecg_img_batch)
                    loss = criterion(outputs, lab_batch)
                    test_loss += loss.item()
    ...
    plt.title("ROC Curve per tutti i fold (opzione 3 - solo ECG)")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Learning Curves (Train/Test Loss per Fold) - opzione 3")
...
if __name__ == "__main__":
    ...
    # Training + CV per il modello 3 (solo ECG 2D)
    train_and_evaluate_3_only(
        signals=signals,
        tabular_data=tabular_data,
        num_epochs=5,       # aumenta a 30–50 quando tutto gira
        batch_size=32,
        n_splits=10,
        threshold=0.5
    )